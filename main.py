    ...
    if request.method == "POST":
        ...
        if email != None:
            try:
                ...
                while True:
                    collection_otp = Otp.objects(email=email).first()
                    if collection_otp == None:
                        break
                    collection_otp.delete()
                collection_otp = Otp()
                collection_otp.email = email
                collection_otp.otp = content
                collection_otp.time = Now_time()
                collection_otp.save()
                otp_id = collection_otp.id
...
def otp_verification(email):
    ...
    if request.method == "POST":
        ...
        collection_otp = Otp.objects(email=email).first()
        collection_otp.to_json()
        print(collection_otp.otp)
        if collection_otp.otp == otp:
            collection_otp.delete()
